...
def validate_email(
    ...
    *,  # subsequent arguments are keyword-only
    ...
    check_existance: Optional[bool] = None,
    ...
    ret.is_valid = True  # assume the email to be valid by default
    ...
    if len(domain_part) == 0:
        ...
        domain_part_info = validate_email_domain_name(
            domain_part,
            test_environment=test_environment,
            globally_deliverable=globally_deliverable,
        )
    ...
    if check_existance and not test_environment:
        from .EmailExistanceValidator import EmailExistenceValidator
